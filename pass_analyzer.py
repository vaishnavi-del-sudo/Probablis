...
# determine directory containing this script so we can load
# data files relative to it regardless of the current working
# directory the user runs the program from.
BASE_DIR = os.path.dirname(os.path.abspath(__file__))

# markov model is stored next to the script
MARKOV_MODEL_FILE = os.path.join(BASE_DIR, "markov_model.json")

# load the model early; give a helpful error if it's missing.
try:
    with open(MARKOV_MODEL_FILE, "r") as f:
        markov_model = json.load(f)
except FileNotFoundError:
    raise FileNotFoundError(f"Markov model file not found at {MARKOV_MODEL_FILE}")
